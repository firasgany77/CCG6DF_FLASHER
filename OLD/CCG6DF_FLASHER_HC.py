...
# -------------------------
DEVICE_MODE_OFFSET = 0x0000  # Where to read device mode: FW vs Bootloader
PD_CONTROL_OFFSET = 0x1006  # Port-0 PD_CONTROL register (HPIv2)
RESPONSE_OFFSET = 0x1400  # Where CCG places the response code
...
JUMP_TO_BOOT_OFFSET = 0x0007  # Where to write the JUMP_TO_BOOT command
JUMP_TO_BOOT_SIGNATURE = 0xA5  # Often the signature or value needed for jump
ENTER_FLASHING_MODE_OFFSET = 0x000A  # Register to enter "Flashing Mode"
FLASH_ROW_READ_WRITE_OFFSET = 0x000C  # Register to trigger flash read/write
FLASH_DATA_AVAILABLE = 0x03  # Response code for "Flash Data Available"
VALIDATE_FW_OFFSET = 0x000B  # Register to request FW validation
RESET_OFFSET = 0x0008  # Register to force a soft reset

DEVICE_MODE_FW = 0x01  # "firmware mode" value
DEVICE_MODE_BOOTLOADER = 0x00  # "bootloader mode" value

FLASH_ROW_SIZE_BYTES = 256  # Typical CCG flash row size
METADATA_ROW_NUMBER = 0xFF80  # Example address of FW1 metadata row
...
def read_event():
    ...
    Some designs store asynchronous events in a queue or same register, adapt as needed.
    """
...
def parse_intel_hex_file(hex_path):
    ...
    for addr in range(min_addr, max_addr + 1):
        ...
        # Example: row_number = address // 256
